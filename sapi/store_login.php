...
$chek = $mysqli->query("select * from rest_details where  email='".$mobile."' and status = 1 and password='".$password."'");
$status = $mysqli->query("select * from rest_details where status = 1");
if($status->num_rows !=0)
{
if($chek->num_rows != 0)
{
    $c = $mysqli->query("select * from rest_details where  email='".$mobile."' and status = 1 and password='".$password."'")->fetch_assoc();
    $curr = $mysqli->query("select * from tbl_setting")->fetch_assoc();
