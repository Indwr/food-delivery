...
$sql_distance = $mysqli->query("SELECT (((acos(sin((".$lats."*pi()/180)) * sin((`lats`*pi()/180))+cos((".$lats."*pi()/180)) * cos((`lats`*pi()/180)) * cos(((".$longs."-`longs`)*pi()/180))))*180/pi())*60*1.1515*1.609344) as distance,id FROM rest_details where status=1 and title like '%".$keyword."%'");
...
while($row = $sql_distance->fetch_assoc())
{
	...
	$returnArr = array("ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"Restaurant Search Data Get Successfully!","restuarant_data"=>$pop);
