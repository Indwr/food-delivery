...
else 
{
  ...
  $counter = $mysqli->query("select * from tbl_fav where uid=".$uid."");
	...
	if($counter->num_rows != 0)
    {
		...
		$sql_distance = $mysqli->query("SELECT (((acos(sin((".$lats."*pi()/180)) * sin((`lats`*pi()/180))+cos((".$lats."*pi()/180)) * cos((`lats`*pi()/180)) * cos(((".$longs."-`longs`)*pi()/180))))*180/pi())*60*1.1515*1.609344) as distance,id FROM rest_details where status=1 and id=".$folk['rest_id']."");
...
while($row = $sql_distance->fetch_assoc())
{
	...
	$returnArr = array("ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"Favourite List Get Successfully!","restuarant_data"=>$pop);
	}
	else {
		$returnArr = array("ResponseCode"=>"401","Result"=>"false","ResponseMsg"=>"Restaurant  Favourite List Not Found!");
	}
}
