...
else 
{
  ...
  if ($unit == "K") {
      ...
      return $miles;
		...
		$sql_distance = $mysqli->query("SELECT (((acos(sin((".$lats."*pi()/180)) * sin((`lats`*pi()/180))+cos((".$lats."*pi()/180)) * cos((`lats`*pi()/180)) * cos(((".$longs."-`longs`)*pi()/180))))*180/pi())*60*1.1515*1.609344) as distance,id FROM rest_details where status=1 and coupon_display!=0");

$pop = array();
	$pol = array();
$counter = $sql_distance->num_rows;
if($counter != 0)
{
while($row = $sql_distance->fetch_assoc())
{
	...
	$returnArr = array("ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"Favourite List Get Successfully!","restuarant_data"=>$pop);
		...
		$returnArr = array("ResponseCode"=>"401","Result"=>"false","ResponseMsg"=>"Not Found Any Near Offer!");
	}
}
